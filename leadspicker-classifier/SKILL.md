...
description: >
  ...
  Use this skill whenever the user wants to classify, score, label, tag,
...
Each column runs Gemini 2.5 Flash on every contact in the project to classify, categorize,
...
The system prompt forces Gemini 2.5 Flash to output strict JSON. Understanding this is critical
...
- NEVER ask Gemini 2.5 Flash to output `yes/no`, `classification: yes/no`, or any custom format
...
- For boolean: Gemini 2.5 Flash returns `yes`/`no` with a message
- For string: Gemini 2.5 Flash returns a text value in the `result` field
...
every prompt — keep it lean so Gemini 2.5 Flash focuses on what matters.
...
## Prompt Writing Guidelines for Gemini 2.5 Flash
...
- **Be direct and specific.** Gemini 2.5 Flash performs best with clear, unambiguous instructions.
...
- **Keep prompts concise.** Gemini 2.5 Flash works well with shorter prompts. Avoid unnecessary
...
- **Handle missing data.** Always include a fallback instruction.
  For boolean: "If there is not enough data to decide, answer no."
...
- Write classification/analytical prompts in English (default). For prompts that generate text output in a specific language (vocatives, icebreakers, salutations), write the task instructions in English but specify the output language. Gemini 2.5 Flash handles multilingual tasks well.
...
If there is not enough information to determine, answer no.
...
### Scored → Boolean Derived Column (reusable pattern)

When a scored classification exists (e.g., "Company Relevance" or "Position Relevance"),
create a follow-up boolean column that reads the scored result and converts it to true/false
based on a threshold.

**Column name:** `Is {Category}` (e.g., `Is Relevant Company`, `Is Relevant Position`)
**`is_boolean`: true**
**Requires:** The scored column must be fully processed before launching this column.

```
You are given the result of a previous classification. The result is in the format
"Yes | score | reasoning" or "No | score | reasoning".
...
Input:
- {Scored Column Name}: {{{scored_column_variable}}}
```

**Example for company relevance:**
- Input variable: `{{company_relevance}}`
- Column name: `Is Relevant Company`

**Example for position relevance:**
- Input variable: `{{position_relevance}}`
- Column name: `Is Relevant Position`

**The threshold (7) can be adjusted per user request** — some users want >= 5 (broader),
others want >= 9 (stricter). Default to 7 unless specified.
...
- Write classification prompts in English by default. Use target language only when the output must be in that language.
